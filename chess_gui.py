...
undo_logger = ChessLogger(__name__, file="gui_logs.log", h="console")
...
def draw_pieces(screen, game_state):
    ...
    for r in range(DIMENSION):
        for c in range(DIMENSION):
            ...
            if piece is not None and piece != EPlayer.EMPTY:
...
def highlight_square(screen, game_state, valid_moves, square_selected):
    ...
    if square_selected != () and game_state.is_valid_piece(square_selected[0], square_selected[1]):
        ...
        if (game_state.whose_turn() and game_state.get_piece(row, col).is_player(EPlayer.PLAYER_1)) or \
                (not game_state.whose_turn() and game_state.get_piece(row, col).is_player(EPlayer.PLAYER_2)):
...
def main():
    ...
    ai_color = EPlayer.PLAYER_2 if human_player == EPlayer.PLAYER_1 else EPlayer.PLAYER_1
            ...
            if e.type == py.QUIT:
                ...
                if not game_over:
                    ...
                    if len(player_clicks) == 2:
                        if (player_clicks[1]) not in valid_moves:
                            square_selected = ()
                            player_clicks = []
                            valid_moves = []
                        else:
                            game_state.move_piece((player_clicks[0]), (player_clicks[1]))
                            square_selected = ()
                            player_clicks = []
                            valid_moves = []
                            if number_of_players == 1:
                                ai_move = ai_bot.minimax(game_state, 3, -100000, 100000, True, ai_color)
            ...
            if number_of_players == 1:
                while True:
                    ...
                    if human_player in ["w", "b"]:
                        human_player = EPlayer.PLAYER_1 if human_player == "w" else EPlayer.PLAYER_2
                        break
                    else:
                        print("Enter w or b.\n")
                break

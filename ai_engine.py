import chess_engine
from enums import EPlayer


class ChessAi:
    def minimax(self, game_state, depth, alpha, beta, maximizing_player, player_color):
        csc = game_state.checkmate_stalemate_checker()

        if maximizing_player:
            if csc == 0:
                return 5000000
            elif csc == 1:
                return -5000000
            elif csc == 2:
                return 100
        else:
            if csc == 1:
                return 5000000
            elif csc == 0:
                return -5000000
            elif csc == 2:
                return 100

        if depth <= 2 or csc != 3:
            return self.evaluate_board(game_state, player_color)

        if maximizing_player:
            return self.maximize(game_state, depth, alpha, beta, player_color)
        else:
            return self.minimize(game_state, depth, alpha, beta, player_color)

    def maximize(self, game_state, depth, alpha, beta, player_color):
        max_evaluation = -10000000
        all_possible_moves = game_state.get_all_legal_moves(player_color)
        best_possible_move = None

        for move_pair in all_possible_moves:
            game_state.move_piece(move_pair[0], move_pair[1], True)
            evaluation = self.minimax(game_state, depth - 1, alpha, beta, False,
                                      EPlayer.PLAYER_1 if player_color == EPlayer.PLAYER_1 else EPlayer.PLAYER_2)
            game_state.undo_move(True)

            if max_evaluation < evaluation:
                max_evaluation = evaluation
                best_possible_move = move_pair

            alpha = max(alpha, evaluation)
            if beta <= alpha:
                break

        if depth == 3:
            return best_possible_move
        else:
            return max_evaluation

    def minimize(self, game_state, depth, alpha, beta, player_color):
        min_evaluation = 10000000
        all_possible_moves = game_state.get_all_legal_moves(player_color)
        best_possible_move = None

        for move_pair in all_possible_moves:
            game_state.move_piece(move_pair[0], move_pair[1], True)
            evaluation = self.minimax(game_state, depth - 1, alpha, beta, True,
                                      EPlayer.PLAYER_1 if player_color == EPlayer.PLAYER_1 else EPlayer.PLAYER_2)
            game_state.undo_move()

            if min_evaluation > evaluation:
                min_evaluation = evaluation
                best_possible_move = move_pair

            beta = min(beta, evaluation)
            if beta <= alpha:
                break

        if depth == 3:
            return best_possible_move
        else:
            return min_evaluation

    def evaluate_board(self, game_state, player_color):
        evaluation_score = 0

        for row in range(0, 8):
            for col in range(0, 8):
                if game_state.is_valid_piece(row, col):
                    evaluated_piece = game_state.get_piece(row, col)
                    evaluation_score += self.get_piece_value(evaluated_piece, player_color)

        return evaluation_score

    def get_piece_value(self, piece, player_color):
        if player_color is EPlayer.PLAYER_1:
            if piece.is_player("black"):
                if piece.get_name() == "k":
                    return -1000
                elif piece.get_name() == "q":
                    return -100
                elif piece.get_name() == "r":
                    return -50
                elif piece.get_name() == "b":
                    return -30
                elif piece.get_name() == "n":
                    return -30
                elif piece.get_name() == "p":
                    return -10
            else:
                if piece.get_name() == "k":
                    return 1000
                elif piece.get_name() == "q":
                    return 100
                elif piece.get_name() == "r":
                    return 50
                elif piece.get_name() == "b":
                    return 30
                elif piece.get_name() == "n":
                    return 30
                elif piece.get_name() == "p":
                    return 10
        else:
            if piece.is_player("white"):
                if piece.get_name() == "k":
                    return 1000
                elif piece.get_name() == "q":
                    return 100
                elif piece.get_name() == "r":
                    return 50
                elif piece.get_name() == "b":
                    return 30
                elif piece.get_name() == "n":
                    return 30
                elif piece.get_name() == "p":
                    return 10
            else:
                if piece.get_name() == "k":
                    return -1000
                elif piece.get_name() == "q":
                    return -100
                elif piece.get_name() == "r":
                    return -50
                elif piece.get_name() == "b":
                    return -30
                elif piece.get_name() == "n":
                    return -30
                elif piece.get_name() == "p":
                    return -10
